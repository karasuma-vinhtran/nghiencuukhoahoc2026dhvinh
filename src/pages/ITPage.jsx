...
import ITResult from "../layouts/ITResult.jsx"
...
function ITPage({ lang, changeLang, onReady }) {
	...
	useEffect(() => {
		...
		if (lang != "vi") {
			fetch(`./lang/${lang}.json`)
				.then(res => res.json())
				.then(data => setLangDisplay(data.it))
		}
		...
		<>
			<div id="home" className="h-auto w-full flex flex-col justify-start items-center">
				<div className="h-30 w-full flex flex-col justify-center items-center">
					<span className="text-black text-3xl font-['Exo_2',_sans-serif]">{lang == "vi" ? "Từ điển chuyên ngành CNTT" : LangDisplay?.text_title}</span>
					<span className="mt-2 w-[85%] text-black text-[15px] font-['Exo_2',_sans-serif] text-center">{lang == "vi" ? "Tra cứu thuật ngữ CNTT nhanh chóng, chính xác, dễ hiểu và kèm giải thích rõ ràng, dễ tiếp cận." : LangDisplay?.text_describe}</span>
				</div>
					...
					<div id="searchbar" className="h-full w-3/4 flex flex-row justify-center items-center transition-all duration-500 sm:w-[calc(100%-140px)]">
						<div className={`ml-3 h-10 w-full text-black flex flex-row rounded-full ${getClassUI()}`}>
							...
							<input
								...
								type="text"
								placeholder={lang == "vi" ? "Nhập từ khóa chuyên ngành tìm kiếm....." : LangDisplay?.search_placeholder}
								autoComplete="off"
							/>
							<X id="search-delete" className="hidden mt-2 ml-1 mr-2 h-6 text-black justify-center items-center" />
						</div>
					</div>
				...
				<div id="home-display" className="relative mt-2 h-[calc(100%-56px-64px-120px)] w-full flex justify-center items-start">
					<div className="relative h-full w-[calc(100%-25px)] overflow-y-auto scrollbar-hide">
						...
						<ITResult lang={lang} textDisplay={TextSearch} langDisplay={LangDisplay} text={TextSearch.toLowerCase()} hidden={ShowResult} />
					</div>
				</div>
			</div>
		</>
